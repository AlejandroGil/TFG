 ...
 * 
 * Paolo Cremonesi, Yehuda Koren, and Roberto Turrin. Performance of recommender
...
public class MyUserNeighborhoodRecommender<U, I> extends FastRankingRecommender<U, I> {
	 ...
	 *            similarity of users
	 * @param tr
	 *            type of transformation (standard, mean centeringo or z-score)
	 * @param normalize
	 *            choose whether the ratings are normalized or not
	 */
	public MyUserNeighborhoodRecommender(FastPreferenceData<U, I> data, UserNeighborhood<U> neighborhood, int q,
			UserSimilarity<U> sim, TRANSFORM tr, boolean normalize) {
		super(data, data);
		this.data = data;
	...
	public Int2DoubleMap getScoresMap(int uidx) {
		...
		scoresMap.forEach((k, v) -> {
			double b2 = normalize ? b / cMap.get(k) : b;
			double s = a + b2 * v;
