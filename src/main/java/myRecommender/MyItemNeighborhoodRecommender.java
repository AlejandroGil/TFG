...
public class MyItemNeighborhoodRecommender<U, I> extends FastRankingRecommender<U, I> {
	 ...
	 *            exponent of the similarity
	 */
	public MyItemNeighborhoodRecommender(FastPreferenceData<U, I> data, ItemNeighborhood<U> neighborhood, int q,
			ItemSimilarity<U> sim, TRANSFORM std, boolean normalize) {
		super(data, data);
		this.data = data;
	...
	public Int2DoubleMap getScoresMap(int uidx) {
		...
		data.getUidxPreferences(uidx).forEach(jp -> {
			...
			neighborhood.getNeighbors(i).forEach(is -> {
				int j = is.v1;
				// System.out.println("Asking similarity for items " + i + " and
				// " + j + " -> " + data.iidx2item(i) + " and " +
				// data.iidx2item(j));
		...
		scoresMap.forEach((i, v) -> {
			...
			final double b = normalize ? t2 / cMap.get(i) : t2;
