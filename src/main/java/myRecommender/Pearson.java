...
public abstract class Pearson implements Similarity {
	...
	public IntToDoubleFunction similarity(int idx) {
		...
		return idx2 -> {
			...
			double prod = data.getUidxPreferences(idx2).filter(p -> map.containsKey(p.v1)).mapToDouble(iv -> {
	...
	public Stream<Tuple2id> similarElems(int idx1) {
		...
		if (data.useIteratorsPreferentially()) {
			if (dense) {
				...
				return range(0, productMap.length).filter(i -> productMap[i] > threshold).mapToObj(i -> {
					double n2a1 = n2a;
				...
				return getFasterProductMap(idx1).int2DoubleEntrySet().stream().filter(e -> e.getValue() > threshold)
						.map(e -> {
							double n2a1 = n2a;
			...
			if (dense) {
				...
				return getProductMap(idx1).int2DoubleEntrySet().stream().filter(e -> e.getValue() > threshold)
						.map(e -> {
							double n2a1 = n2a;
							double n2a2 = e.getValue();
							if (commonNorm) {
	...
	private double getNorm2(int idx) {
		return data.getUidxPreferences(idx).mapToDouble(IdxPref::v2).map(x -> Math.pow(x - stats.get(idx).getMean(), 2))
				.sum();
	}
	...
	private Int2DoubleMap getNorm(int idx1, int idx2) {
		...
		data.getUidxPreferences(idx2).filter(p -> map.containsKey(p.v1)).forEach(iv -> {
	...
	private Int2DoubleMap getFasterProductMap(int uidx) {
		...
		while (iidxs.hasNext()) {
			...
			while (vidxs.hasNext()) {
				productMap.addTo(next,
						(iv - stats.get(uidx).getMean()) * (vvs.nextDouble() - stats.get(next).getMean()));
			}
		}
